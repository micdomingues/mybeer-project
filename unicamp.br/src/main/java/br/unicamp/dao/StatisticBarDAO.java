...
public class StatisticBarDAO extends ConnectionFactory
{
	...
	public StatisticBar carregar(int codbar)
	{
		...
		try
		{
			pstmt = conexao.prepareStatement("SELECT BAR.CODBAR, BAR.CNPJ, BAR.NOME, BAR.NOMEFANTASIA, AVG(PRECO), "
					+ "AVG(QUALIDADE), (SELECT COUNT(*) FROM AVALIACAO AV2 WHERE FAVORITO = 1 AND AV2.CODBAR = AV1.CODBAR) AS FAVORITOS "
